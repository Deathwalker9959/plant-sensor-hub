...
import { AlertData, GroupData, useData } from "../providers/data/DataContext";
import { ScrollView } from "react-native-gesture-handler";
import Accordion, { AccordionProps } from "react-native-collapsible/Accordion";
type Props = {
	navigation: StackNavigationProp<ParamListBase, "pages/alerts">;
	route: RouteProp<ParamListBase, "pages/alerts">;
};

const Alerts: React.FC<Props> = ({ navigation, route }: Props) => {
	//@ts-ignore
	...
	const navigateGroupAdd = () => {
		navigation.navigate("pages/alerts_add");
	};
			...
			<View style={styles.bodyRow}>
				<ScrollView
					...
					{alerts ? alerts.map((alert, i) => {
						const sensor: SensorData = sensorData.find((sensor) => sensor.id == alert.sensorId);
						return (
							<>
								<View
									key={i}
									...
									<View
										...
										/>
										<Text
											style={{ ...globalStyles.textPrimary, paddingLeft: 20 }}
											key={`${i}_text`}>
											{sensor?.title ?? ""}
										</Text>
									</View>
									<Text style={{...globalStyles.textPrimary, color: "", fontSize:20, textAlignVertical:"center", textAlign:"center"}}>{alert.operator}</Text>
									<Text style={{...globalStyles.textPrimary, color: "black",  textAlignVertical:"center", textAlign:"right"}}>{alert.alertValue}{sensor.unit}</Text>
								</View>
